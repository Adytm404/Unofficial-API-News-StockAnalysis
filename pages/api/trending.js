...
const TRENDING_URL = 'https://stockanalysis.com/trending/'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const query = hydrationPayload?.[1]?.data?.query
    const rawItems = hydrationPayload?.[1]?.data?.data

    if (!Array.isArray(rawItems)) {
      throw new Error('Trending data not found in hydration payload')
    }

    const data = rawItems.map(mapTrendingItem)
